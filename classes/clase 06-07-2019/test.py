...
if __name__ == "__main__":
	...
	# ]
	graphNodes=[
		["a", {"color":"silver", "pos":np.array([1, 2])}], 
		["b", {'color':"silver", "pos":np.array([2, 2])}],
		["c", {'color':"silver", "pos":np.array([0.5, 1])}],
		["d", {"color":"silver", "pos":np.array([2.5, 1])}], 
		["e", {"color":"silver", "pos":np.array([1, 0])}], 
		["f", {"color":"silver", "pos":np.array([2, 0])}]
	]
	...
	#nx.set_node_attributes(G, "silver", 'color')
	#nx.set_edge_attributes(G, "black", "color")
	hamiltonList=list(nx.algorithms.tournament.hamiltonian_path(G))
	print(hamiltonList)
	G.nodes[hamiltonList[0]]["color"]="orange"
	G.nodes[hamiltonList[-1]]["color"]="limegreen"
	for enum, nodeI in enumerate(hamiltonList):
		if(enum==len(hamiltonList)-1):
			break
		G.edges[hamiltonList[enum], hamiltonList[enum+1]]["color"]="red"
